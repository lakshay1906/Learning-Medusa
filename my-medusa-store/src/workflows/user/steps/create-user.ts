...
export const createUserStep = createStep(
  "create-user-step",
  async ({ actor_type, ...data }: CreateUserStepInput, { container }) => {
    if (actor_type === "restaurant") {
      const service = container.resolve(RESTAURANT_MODULE);

      const restaurantAdmin = await service.createRestaurantAdmins(data);
  ...
  function ({ id, actor_type }: any, { container }) {
    if (actor_type === "restaurant") {
      const service = container.resolve(RESTAURANT_MODULE);
